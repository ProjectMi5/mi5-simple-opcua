...
class OpcuaClientVariable extends EventEmitter {
  ...
  init() {
    let self = this;
    if (self.subscribedFromBeginning) self.subscribe();
    self.client
      .readDatatype(self.nodeId)
      .then(function(value) {
        self.dataType = value;
  ...
  subscribe() {
    let self = this;
    if (this.monitoredItem) return self.debug('is already being monitored.');
  ...
  async write(value) {
    let self = this;
    if (this.initialized) {
      return await self.client.writeNodeValue(
        self.nodeId,
        value,
        self.dataType
      );
      ...
      return new Promise(function(resolve, reject) {
        self.once('init', function() {
          self.client
            .writeNodeValue(self.nodeId, value, self.dataType)
            .then(resolve);
        });
      });
    }
  ...
  onChange(callback) {
    if (!this.monitoredItem) this.subscribe();
  ...
  oneChange(callback) {
    ...
    this.once('change', function(value) {
      if (self.subscribedFromBeginning === false) this.unsubscribe();
